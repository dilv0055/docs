...
2. Add a **\<binding>** element and set its `name` attribute to an appropriate value. The example uses the name `MessageSecurity`.

3. Add a **\<security>** element and set the `mode` attribute to `Message`.

4. Within the **\<security>** element, add a **\<message>** element and set the `clientCredentialType` attribute to `Certificate`.
